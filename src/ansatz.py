...
def apply_sel_with_matched_pauli(weights, noise_p: float, noise_mask=None):
    """
    ...
    # Optional targeted-noise mask. Shape must match weights: (n_layers, n_qubits, 3).
    # mask[..., 0], mask[..., 1], mask[..., 2] correspond to RZ, RY, RZ.
    # A value of 1 applies the scheduled Pauli noise; 0 disables it for that gate.
    if noise_mask is not None:
        mask_shape = getattr(noise_mask, "shape", None)
        weights_shape = getattr(weights, "shape", None)
        if tuple(mask_shape) != tuple(weights_shape):
            raise ValueError(f"noise_mask shape {mask_shape} must match weights shape {weights_shape}.")

    def _mask_value(layer_idx, wire, rot_idx):
        if noise_mask is None:
            return 1.0
        return float(noise_mask[layer_idx, wire, rot_idx])
    ...
    for layer_idx in range(N_LAYERS):
        ...
        for wire in WIRES:
            ...
            p_phi = paulierror_prob * _mask_value(layer_idx, wire, 0)
            if p_phi > 0.0:
                qml.PauliError("Z", p_phi, wires=wire)
            qml.RZ(phi, wires=wire)

            p_theta = paulierror_prob * _mask_value(layer_idx, wire, 1)
            if p_theta > 0.0:
                qml.PauliError("Y", p_theta, wires=wire)
            qml.RY(theta, wires=wire)

            p_omega = paulierror_prob * _mask_value(layer_idx, wire, 2)
            if p_omega > 0.0:
                qml.PauliError("Z", p_omega, wires=wire)
