...
_AVAILABLE_EXPERIMENT_MODES = [
    ...
    "targeted_standard",
    "targeted_method_suite_sample",
    "targeted_method_suite_aggregate",
]
...
def parse_args():
    ...
    # --- Targeted Pauli-noise experiments ---
    p.add_argument(
        "--target-score-steps",
        type=int,
        default=C.TARGET_SCORE_STEPS,
        help="number of clean warm-up steps used only to estimate target-gate importance",
    )
    p.add_argument(
        "--target-score-type",
        type=str,
        default=C.TARGET_SCORE_TYPE,
        choices=["mean_abs_grad"],
        help="importance score used to select targeted Pauli-noise gates",
    )
    p.add_argument(
        "--target-fractions",
        type=float,
        nargs="+",
        default=C.TARGET_FRACTIONS,
        help="fractions of parameters to target; ceil(fraction * num_params) is selected",
    )
    p.add_argument(
        "--target-selection-modes",
        type=str,
        nargs="+",
        default=C.TARGET_SELECTION_MODES,
        choices=["top", "bottom", "random"],
        help="target-set construction modes. For fraction=1.0 only top is kept to avoid duplicate all-gate baselines",
    )
    p.add_argument(
        "--target-suite-schedules",
        type=str,
        nargs="+",
        default=C.TARGET_SUITE_SCHEDULES,
        choices=["fixed_step", "manual"],
        help="which Pauli schedules to include in targeted_method_suite_sample",
    )
    p.add_argument("--target-random-base-seed", type=int, default=C.TARGET_RANDOM_BASE_SEED)
    p.add_argument(
        "--target-no-save-masks",
        dest="target_save_masks",
        action="store_false",
        default=C.TARGET_SAVE_MASKS,
        help="do not save per-seed target scores/masks in targeted modes",
    )
    p.add_argument(
        "--target-include-shots",
        dest="target_include_shots",
        action="store_true",
        default=C.TARGET_INCLUDE_SHOTS,
        help="include shot-noise baselines in targeted_standard",
    )
...
def apply_args(args):
    ...
    # Targeted Pauli-noise experiments
    C.TARGET_SCORE_STEPS = args.target_score_steps
    C.TARGET_SCORE_TYPE = args.target_score_type
    C.TARGET_FRACTIONS = list(args.target_fractions)
    C.TARGET_SELECTION_MODES = list(args.target_selection_modes)
    C.TARGET_SUITE_SCHEDULES = list(args.target_suite_schedules)
    C.TARGET_RANDOM_BASE_SEED = args.target_random_base_seed
    C.TARGET_SAVE_MASKS = bool(args.target_save_masks)
    C.TARGET_INCLUDE_SHOTS = bool(args.target_include_shots)
