...
def make_regularized_cost_for_hamiltonian(hamiltonian, noise_mode=None):
    ...
    if mode == "matched":
        @qml.qnode(dev, interface="autograd", diff_method="best")
        def _masked_cost(weights, noise_strength, noise_mask):
            ansatz_fn(weights, noise_strength, noise_mask=noise_mask)
            return qml.expval(hamiltonian)

        def cost(weights, noise_strength, noise_mask=None):
            if noise_mask is None:
                noise_mask = np.ones(tuple(weights.shape), dtype=float)
            return _masked_cost(weights, noise_strength, noise_mask)

        return cost

    @qml.qnode(dev, interface="autograd", diff_method="best")
    def _layerwise_cost(weights, noise_strength):
        ansatz_fn(weights, noise_strength)
        return qml.expval(hamiltonian)

    def cost(weights, noise_strength, noise_mask=None):
        if noise_mask is not None:
            raise ValueError("Targeted noise masks are currently supported only for noise_mode='matched'.")
        return _layerwise_cost(weights, noise_strength)
...
def make_pauli_cost(noise_strength: float, noise_mask=None):
    """Freeze the user-facing regularization strength and return a unary cost function."""
    def cost(weights):
        return REGULARIZED_COST(weights, noise_strength, noise_mask=noise_mask)
    return cost
