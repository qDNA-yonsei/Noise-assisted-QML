...
from ..targeting import compute_clean_warmup_scores, make_target_mask, mask_metadata, random_mask_seed, target_method_specs
...
def _method_specs() -> list[dict]:
    if C.EXPERIMENT_MODE.startswith("targeted_method_suite"):
        return target_method_specs(include_shots=True)

    specs = [
        {
            "method_order": 1, "method_key": "clean", "method_label": "clean", "method_group": "clean",
            "schedule_mode": "", "shots": "", "target_selection_mode": "", "target_fraction": ""
        },
        {
            "method_order": 2, "method_key": "pauli_fixed", "method_label": "pauli_fixed", "method_group": "pauli",
            "schedule_mode": "fixed_step", "shots": "", "target_selection_mode": "", "target_fraction": ""
        },
        {
            "method_order": 3, "method_key": "pauli_manual", "method_label": "pauli_manual", "method_group": "pauli",
            "schedule_mode": "manual", "shots": "", "target_selection_mode": "", "target_fraction": ""
        },
    ]
    next_order = 4
...
def _run_pauli_fixed_seed_once(init_p, clean_cost, regularized_cost, noise_mask=None) -> dict:
    ...
    def dynamic_cost(weights):
        return regularized_cost(weights, current_noise["value"], noise_mask=noise_mask)
...
def _run_pauli_manual_seed_once(init_p, clean_cost, regularized_cost, noise_mask=None) -> dict:
    params = to_trainable(init_p)
    ...
    for noise_value in schedule:
        ...
        if use_clean:
            ...
            def cost_fn(weights, _current_noise=current_noise):
                return regularized_cost(weights, _current_noise["value"], noise_mask=noise_mask)
...
def _build_campaign_manifest() -> dict:
    ...
    return {
        ...
        "target_fractions": [float(x) for x in C.TARGET_FRACTIONS],
        "target_selection_modes": list(C.TARGET_SELECTION_MODES),
        "target_suite_schedules": list(C.TARGET_SUITE_SCHEDULES),
        "target_random_base_seed": int(C.TARGET_RANDOM_BASE_SEED),
...
def _ensure_campaign_scaffold() -> None:
    ...
    if cfg_path.exists():
        ...
        keys_to_check = [
            ...
            "target_suite_schedules",
            "target_random_base_seed",
    ...
    if not mm_path.exists():
        _write_csv(mm_path, _method_manifest_rows(), ["method_order", "method_key", "method_label", "method_group", "schedule_mode", "shots", "target_selection_mode", "target_fraction"])
...
def run_method_suite_sample() -> dict:
    ...
    _write_csv(sdir / "method_manifest.csv", _method_manifest_rows(), ["method_order", "method_key", "method_label", "method_group", "schedule_mode", "shots", "target_selection_mode", "target_fraction"])
    ...
    target_score_cache: dict[int, object] = {}
    target_mask_dir = sdir / "target_masks"
    if C.EXPERIMENT_MODE.startswith("targeted_method_suite") and C.TARGET_SAVE_MASKS:
    ...
    for method_idx, spec in enumerate(method_specs, start=1):
        ...
        for seed_idx, seed in enumerate(C.SEARCH_SEEDS, start=1):
            p0 = init_params(seed)
            target_meta = {
                "target_selection_mode": spec.get("target_selection_mode", ""),
                "target_fraction": spec.get("target_fraction", ""),
            ...
            elif spec["method_group"] == "targeted_pauli":
                if C.NOISE_MODE != "matched":
                    raise ValueError("targeted_method_suite currently supports only --noise-mode matched.")
                if int(seed) not in target_score_cache:
                    score_result = compute_clean_warmup_scores(p0, clean_cost, C.TARGET_SCORE_STEPS)
                    target_score_cache[int(seed)] = score_result
                    if C.TARGET_SAVE_MASKS:
                        np.save(target_mask_dir / f"seed{int(seed):05d}_target_scores.npy", score_result.scores)
                        np.save(target_mask_dir / f"seed{int(seed):05d}_target_score_hist.npy", score_result.score_hist)
                        np.save(target_mask_dir / f"seed{int(seed):05d}_warmup_clean_eval_hist.npy", score_result.clean_eval_hist)
                score_result = target_score_cache[int(seed)]
                sel_mode = str(spec["target_selection_mode"])
                frac = float(spec["target_fraction"])
                rseed = None
                if sel_mode == "random":
                    rseed = random_mask_seed(training_seed=int(seed), fraction=frac, sample_index=sample_index, method_key=spec["method_key"])
                mask = make_target_mask(score_result.scores, selection_mode=sel_mode, fraction=frac, random_seed=rseed)
                target_meta = mask_metadata(mask, selection_mode=sel_mode, fraction=frac, random_seed=rseed)
                if C.TARGET_SAVE_MASKS:
                    np.save(target_mask_dir / f"seed{int(seed):05d}_{spec['method_key']}_mask.npy", mask)
                if spec["schedule_mode"] == "fixed_step":
                    res = _run_pauli_fixed_seed_once(p0, clean_cost, regularized_cost, noise_mask=mask)
                elif spec["schedule_mode"] == "manual":
                    res = _run_pauli_manual_seed_once(p0, clean_cost, regularized_cost, noise_mask=mask)
                else:
                    raise ValueError(f"Unsupported targeted schedule_mode={spec['schedule_mode']!r}")
    ...
    detailed_fieldnames = [
        ...
        "target_selection_mode", "target_fraction", "target_selected_count", "target_total_params", "target_random_seed", "target_score_steps", "target_score_type",
    ...
    summary_fieldnames = [
        "campaign_tag", "sample_id", "sample_index", "method_order", "method_key", "method_label", "method_group", "schedule_mode", "shots",
        "target_selection_mode", "target_fraction",
