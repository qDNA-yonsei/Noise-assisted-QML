...
def _run_regularized_manual(init_params, noise_mask=None, run_label: str | None = None) -> PauliAnnealingResult:
    """
    ...
    print(
        f"\n[Regularized optimization{(' | ' + run_label) if run_label else ''} | mode={C.NOISE_MODE} | schedule_mode=manual | "
...
def _run_regularized_fixed_step(init_params, noise_mask=None, run_label: str | None = None) -> PauliAnnealingResult:
    """
    ...
    print(
        f"\n[Regularized optimization{(' | ' + run_label) if run_label else ''} | mode={C.NOISE_MODE} | schedule_mode=fixed_step | "
...
def run_pauli_annealing(init_params, noise_mask=None, run_label: str | None = None) -> PauliAnnealingResult:
    if noise_mask is not None and C.NOISE_MODE != "matched":
        raise ValueError("Targeted Pauli-noise masks are currently supported only for noise_mode='matched'.")
    if C.SCHEDULE_MODE == "manual":
        return _run_regularized_manual(init_params, noise_mask=noise_mask, run_label=run_label)
    if C.SCHEDULE_MODE == "fixed_step":
        return _run_regularized_fixed_step(init_params, noise_mask=noise_mask, run_label=run_label)
